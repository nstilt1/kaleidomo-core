// kaleidomo-core/src/backends/avx2.rs
#[cfg(target_arch = "x86")]
...
impl KaleidoBackend for __m256 {
    ...
    unsafe fn scale(self, factor: Self) -> Self {
        unsafe { _mm256_mul_ps(self, factor) }
    }
    #[target_feature(enable = "avx2")]
    #[inline]
    ...
    unsafe fn store_pixel(
        ...
        unsafe {
            ...
            if bilinear {
                ...
                _mm256_storeu_ps(raw_x.as_mut_ptr(), sx);
                _mm256_storeu_ps(raw_y.as_mut_ptr(), sy);
    ...
    unsafe fn store_pixel_hue_shift(
        ...
        unsafe {
            if bilinear {
                ...
                _mm256_storeu_ps(raw_x.as_mut_ptr(), sx);
                _mm256_storeu_ps(raw_y.as_mut_ptr(), sy);
                let hue_shift_degrees = _mm256_cvtss_f32(hue_shift_vec);
